...
- 字幕加载与选择：自动发现同级 .srt/.vtt/.ass 字幕，播放页「字幕」菜单可切换语言或关闭（Media3 内核流式加载，继承同一服务器鉴权；libVLC 暂不支持字幕）
...
- 离线缓存：在文件菜单点"下载"保存到本地，播放时自动使用缓存文件（设置页可查看/删除）
...
20 个测试文件、128 个单元测试（P0 基线 60 + P1 新增 24 + 倍速契约 5 + 缓存优先刷新 5 + 文件格式化 7 + 离线缓存 2 + 字幕发现 10 + 变更后缓存失效 1 + 缓存文件名 SHA-256 键 3 + 播放列表追加去重 1 + 播放列表替换收敛 2 + 删除当前项索引兜底 3 + 断点续播 3 + 删除服务器清理归属数据 2），覆盖媒体类型识别、路径编码、播放列表顺序/循环/随机、断点续播（保存+读取 seek）、多账户切换、长按目录自动识别视频、倍速引擎契约、目录缓存 TTL 条件刷新、变更后缓存失效触发重新拉取、文件大小格式化、离线缓存实体映射、离线缓存文件名 SHA-256 键确定性/无碰撞、播放列表追加去重、播放列表替换仅清当前服务器、删除当前播放项后索引兜底、删除服务器清理其播放列表项与断点（无孤儿行、无跨服务器误删）、字幕扩展名/MIME/语言识别与同级筛选等。

## 已知限制 / 后续（P2，第一版未做）

- 投屏（Chromecast / DLNA）
- libVLC 内核字幕支持（当前字幕仅 Media3 内核可用）
