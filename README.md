...
18 个测试文件、113 个单元测试（P0 基线 60 + P1 新增 24 + 倍速契约 5 + 缓存优先刷新 5 + 文件格式化 7 + 离线缓存 2 + 字幕发现 10），覆盖媒体类型识别、路径编码、播放列表顺序/循环/随机、断点续播、多账户切换、长按目录自动识别视频、倍速引擎契约、目录缓存 TTL 条件刷新、文件大小格式化、离线缓存实体映射、字幕扩展名/MIME/语言识别与同级筛选等。
